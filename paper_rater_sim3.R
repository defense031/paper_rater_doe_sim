...
# ----- LP-based Approximate Matching Assignment Function with Fallback -----
...
assign_papers_matched_approx <- function(n_papers, graders, min_papers, max_papers, diff_threshold = diff_threshold) {
  ...
  # Build candidate pair list: only include pairs with load difference <= diff_threshold.
  pair_list <- list()
  pair_index <- 1
  for(a in 1:(m-1)) {
    for(b in (a+1):m) {
  ...
  if(lp_result$status != 0) {
    warning("Approximate LP did not find an optimal solution; falling back to full candidate pair list.")
    pair_list <- list()
    pair_index <- 1
    for(a in 1:(m-1)) {
      for(b in (a+1):m) {
    ...
    b_full <- c(b_vec, b_total)
    lp_result <- lp("min", cost, A_full, rep("=", length(b_full)), b_full, all.int = TRUE)
    if(lp_result$status != 0) stop("Fallback LP did not find an optimal solution.")
  }
  
  y <- lp_result$solution
...
for(good_prop in good_prop_values) {
  ...
  sensitivity_results <- rbind(sensitivity_results,
                               data.frame(good_prop = good_prop, method = "Random", version = "Raw",
                                          ...
                                          macro_F1 = compute_multinomial_F1(paper_rand$true_rating, paper_rand$avg_raw_score)$macro_f1),
                               ...
                               data.frame(good_prop = good_prop, method = "Matched", version = "Raw",
                                          ...
                                          macro_F1 = compute_multinomial_F1(paper_match$true_rating, paper_match$avg_raw_score)$macro_f1),
...
# Plot the heatmap using geom_tile with stat = "identity".
